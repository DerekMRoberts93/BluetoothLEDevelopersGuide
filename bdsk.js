// Required to run, bluetooth LE, and microchip ADC, and raspberryPi GPIO
var bleno = require('bleno');
var mcpadc = require('mcp-spi-adc');
var Gpio = require('onoff').Gpio;

// create some variables for the LEDS and the Buzzer
var led1 = new Gpio(17, 'out');
var led2 = new Gpio(27, 'out');
var led3 = new Gpio(22, 'out');
var buzzer = new Gpio(18, 'out');

// link loss is the connection between smart phone and Pi, there are 3 alert levels
var link_loss_alert_level = 0;
var immediate_alert_level = 0;

// variables to track temperature 
var temperature_timer;
var celsius = 0.00;

// variables to handle LEDS and beeps to deal with connection alerts/states
var flash_count = 0;
var flash_state = 1;
var beep_count = 0;
var beep_state = 1;
...
// printing to the console current state of the BLE server 
	...
	if (!error) {
		bleno.setServices ([
			...
			new bleno.PrimaryService({
				...
				characteristics: [
					// Client Proximity
					new bleno.Charateristic({ 
						value: 0,
						uuid: '3E099911293F11E493BDAFD0FE6D1DFD',
						properties: ['writeWithoutResponse'],
						onWriteRequest: function (data, offset, withResponse, callback) {
...
// accepted connection, setup variables to zeros
...
// disconnected from the smart phone, alert and flash			
...
// Flash function, input what led to turn on, how long, and flash times			
function startFlashing(led, intervale, times) {
	flash_count = times * 2;
...
function sampleTemperatureSensor(updateValueCallback) {
	var tempSensor = mcpadc.open(0, { speedHz: 1200000 }, function (err) {
		if(err) throw err;
		temperature_timer = setInterval(function () {
			// sensor provides a voltage output that is lineraly proportional to the celsius (centigrade) temperature.
			// TMP36 characteristics: 500mW offset, scaling 10mV per degree C.
			// Example: 750 mV = 25 degrees C
			//   750mV - 500mV = 250mV
